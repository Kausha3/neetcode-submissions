    ...
    private boolean recursion(String s1, String s2, String s3, int s1Len, int s2Len, int s3Len) {
        if (s1Len == 0) {
            if (s2Len == 0) {
                ...
                if(s2.charAt(s2Len - 1) == s3.charAt(s3Len - 1)) {
            ...
            if (s1Len == 0) {
                ...
                if (s1.charAt(s1Len - 1) == s3.charAt(s3Len - 1)) {
        ...
        else if (s1.charAt(s1Len - 1) == s3.charAt(s3Len - 1)) {
            return recursion(s1, s2, s3, s1Len - 1, s2Len, s3Len - 1);
        }
        else if (s2.charAt(s2Len - 1) == s3.charAt(s3Len - 1)) {
            return recursion(s1, s2, s3, s1Len, s2Len - 1, s3Len - 1);
        }
        else {
            return false;
